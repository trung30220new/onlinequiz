...
def admin_dashboard_view(request):
    dict = {
        ...
        'total_teacher':
        TMODEL.Teacher.objects.all().filter(status=True).count(),
...
def admin_teacher_view(request):
    dict = {
        ...
        TMODEL.Teacher.objects.all().filter(status=True).count(),
        'pending_teacher':
        TMODEL.Teacher.objects.all().filter(status=False).count(),
        'salary':
        TMODEL.Teacher.objects.all().filter(status=True).aggregate(
...
def admin_view_teacher_view(request):
    ...
    return render(request, 'quiz/admin_view_teacher.html',
                  {'teachers': teachers})
...
def update_teacher_view(request, pk):
    ...
    if request.method == 'POST':
        ...
        teacherForm = TFORM.TeacherForm(request.POST,
                                        request.FILES,
                                        instance=teacher)
...
def admin_view_pending_teacher_view(request):
    ...
    return render(request, 'quiz/admin_view_pending_teacher.html',
                  {'teachers': teachers})
...
def approve_teacher_view(request, pk):
    ...
    return render(request, 'quiz/salary_form.html',
...
def admin_view_teacher_salary_view(request):
    teachers = TMODEL.Teacher.objects.all().filter(status=True)
    return render(request, 'quiz/admin_view_teacher_salary.html',
                  {'teachers': teachers})
...
def update_student_view(request, pk):
    ...
    if request.method == 'POST':
        ...
        studentForm = SFORM.StudentForm(request.POST,
                                        request.FILES,
                                        instance=student)
...
def admin_add_course_view(request):
    ...
    return render(request, 'quiz/admin_add_course.html',
...
def admin_view_student_marks_view(request):
    students = SMODEL.Student.objects.all()
    return render(request, 'quiz/admin_view_student_marks.html',
                  {'students': students})
...
def admin_view_marks_view(request, pk):
    courses = models.Course.objects.all()
    response = render(request, 'quiz/admin_view_marks.html',
                      {'courses': courses})
...
def admin_check_marks_view(request, pk):
    ...
    results = models.Result.objects.all().filter(exam=course).filter(
        student=student)
...
def destroy(request):
    ...
    print(user.pk)
    print(user.id)
    teacher = TMODEL.Teacher.objects.get(id=user.teacher.id)
    print(teacher)
    print('ok')
    user.teacher.delete()
    user.delete()
...
def init(request):
    ...
    # Create the Teacher object linked to the User
    teacher = TMODEL.Teacher.objects.create(user=user, address=address)
...
def new_login(request):
    students = SMODEL.Student.objects.all()
    return render(request, 'new/login.html', {'students': students})
